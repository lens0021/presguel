...
- [x] 입력 항목 전환(IME_SWITCH 의 `!A` 식 평가: 0이면 1, 아니면 0) + 로마자 항목 글쇠 리매핑(드보락 등)
- [x] 설정창(GTK) + 간단 모드(한글/영문 항목 직접 지정, 단축키를 영문 배치로 변환)
- [ ] 한자 변환(C0 명령), 3개 이상 항목을 직접 고르는 전환 글쇠
...
- `IME_SWITCH` 글쇠(**한글 키 / CapsLock** 등)는 그 단축글쇠의 `value` 식(예 `!A`)을 평가해
  ...
  아니면 0"(0↔1 토글, 그 외 항목에선 0으로)입니다. 한글 항목은 조합, 로마자 항목은 글쇠
  리매핑, 키표 없는 항목은 패스스루합니다.
...
GNOME 입력 소스 목록에서 presguel 옆 **톱니바퀴(설정)** 를 누르면 설정창이 뜹니다
(또는 `presguel-setup` 실행). 설정은 `~/.config/presguel/config.ini` 에 저장됩니다.

- **끔(기본)**: 설정의 **모든 InputEntry** 를 읽어 날개셋과 똑같이 동작합니다.
- **간단 모드 켬**: 드롭다운으로 두 항목을 고릅니다.
  - **한글 입력 항목** — 실제로 쓸 한글 자판.
  - **영문 배치 항목** — 그 자판이 깔린 영문 배열(예: 드보락). 한글 조합 중이든 아니든
    `Ctrl/Alt/Super+키` **단축키를 이 배열로 변환**해 응용에 넘깁니다. 예: 키보드 배열이
    QWERTY(`us`)여도 `Alt+P` 가 드보락 자리 기준으로 전달됩니다.
